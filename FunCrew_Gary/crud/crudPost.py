...
def posts_method(sortmethod):
    ...
    if sortmethod == 'newest':
        ...
        # 關閉資料庫連線
        con.close()
    return render_template("posts.html", nickname=get_nickname(userID), posts=posts, userID=userID, method=sortmethod)
...
def post_detail(postID):
    ...
    try:
        cur.execute("INSERT INTO temp_{}_view_count (postID) VALUES (?)".format(userID), (postID,))
        view = view+1
    ...
    return render_template('post_detail.html', post=post, comments=comments, postID=postID, userID=userID, views=view, previous_url=sortmethod)
# , comments=comments
