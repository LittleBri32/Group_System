...
def personal_info():
    ...
    cur.execute("SELECT reputationScore,birth,email, gender FROM User WHERE userID=?", (userID,))
    score,birth,user_email, gender = cur.fetchone()
    ...
    return render_template('personalInfo.html', nickname=get_nickname(userID),score=score,birth=birth,gender=gender,user_email=user_email, posts=posts,activitys=activitys,user_image=user_image,user_id = str(userID))
...
def info(userID):
    ...
    cur.execute("SELECT reputationScore,cellphone,gender FROM User WHERE userID=?", (userID,))
    score,cellphone, gender = cur.fetchone()

    cur.execute("SELECT image FROM User WHERE userID=?", (userID,))
    user_image = cur.fetchone()

    cur.execute("SELECT postID, postTitle FROM Post WHERE postUserID=? ORDER BY postTime DESE LIMIT 3", (userID,))
    posts = cur.fetchall()
