...
def personal_info():
    ...
    cur.execute("SELECT email, gender FROM User WHERE userID=?", (userID,))
    user_email, gender = cur.fetchone()

    cur.execute("SELECT image FROM User WHERE userID=?", (userID,))
    user_image = cur.fetchone()

    cur.execute("SELECT postTitle FROM Post WHERE postUserID=?", (userID,))
    posts = cur.fetchall()

    cur.execute("SELECT title FROM Activity WHERE organizerUserID=?", (userID,))
    activitys = cur.fetchall()

    return render_template('personalInfo.html', nickname=get_nickname(userID),gender=gender,user_email=user_email, posts=posts,activitys=activitys,user_image=user_image)
