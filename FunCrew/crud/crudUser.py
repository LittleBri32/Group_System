...
def personal_info():
    ...
    # 從人的生日抓年份
    cur.execute(
        "SELECT strftime('%Y', birth) AS birth_year FROM User WHERE userID=?", (userID,)
    )
    birth_year = cur.fetchall()[0]
    ...
    cur.execute("SELECT * FROM Activity WHERE organizerUserID=?", (userID,))
    activities = cur.fetchall()

    cur.execute(
        "SELECT * FROM Activity, Participant WHERE participantUserID=? AND activityID=participantActivityID",
        (userID,),
    )
...
def info(userID):
    ...
    cur.execute(
        "SELECT postID, postTitle FROM Post WHERE postUserID=? ORDER BY postTime DESC LIMIT 3",
        (userID,),
    )
    posts = cur.fetchall()
