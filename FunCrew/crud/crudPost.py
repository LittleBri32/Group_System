...
def post_detail(postID):
    ...
    cur.execute(
        "SELECT * FROM Comment, User WHERE commentPostID = ? AND userID = commentUserID ORDER BY commentTime DESC",
        (postID,),
    )
...
def delete_post(postID):
    ...
    return render_template(
        "delete_post.html", postID=postID, previous_url=request.referrer
    )
