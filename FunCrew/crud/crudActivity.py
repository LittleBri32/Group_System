...
def show_activities_method(show_methed):
    ...
    if show_methed == "signingUp" or show_methed == "None":
        show_methed = "signingUp"
        con = sql.connect("funCrew_db.db")
        con.row_factory = sql.Row
        cur = con.cursor()
        # 從資料庫中獲取所有報名中活動內容
        cur.execute(
            "SELECT * FROM Activity, User WHERE expireDate >= ? AND organizerUserID = userID ORDER BY createTime DESC",
            (postDate,),
        )
        ...
        # 從資料庫中獲取所有籌備中活動內容
        cur.execute(
            "SELECT * FROM Activity, User WHERE expireDate < ? AND time >= ? AND organizerUserID = userID ORDER BY createTime DESC",
            (
                postDate,
                postDate,
            ),
        ...
        # 從資料庫中獲取所有已結束活動內容
        cur.execute(
            "SELECT * FROM Activity, User WHERE time < ? AND organizerUserID = userID ORDER BY createTime DESC",
            (postDate,),
        )
        activities = cur.fetchall()
        # 關閉資料庫連線
...
def activity_detail(activityID):
    ...
    # 從資料庫抓這個活動的參與名單
    cur.execute(
        "SELECT * FROM Activity, Participant WHERE activityID=participantActivityID AND activityID=?",
        (activityID,),
    )
    ...
    # 從資料庫抓這個活動的參與的人
    cur.execute(
        "SELECT nickname FROM User, Participant WHERE userID=participantUserID AND participantActivityID=?",
        (activityID,),
    )
    participants_menber = cur.fetchall()

    # 從資料庫中計算目前參加這個活動的人數有多少
    cur.execute(
        "SELECT COUNT(*) AS count_participants FROM Activity JOIN Participant ON Activity.activityID = Participant.participantActivityID WHERE Activity.activityID =?",
        (activityID,),
    )
    count_participants = cur.fetchone()
    ...
    try:
        return render_template(
            ...
            participants_menber=participants_menber,
        )
...
def submitActivity(id):
    try:
        ...
        with sql.connect("funCrew_db.db") as con:
            ...
            if id == "create":
                ...
                cur.execute(
                    "INSERT INTO Activity \
                            ...
                            peopleLimited, expireDate, fee, organizerUserID, createTime) \
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (
                        id,
                        title,
                ...
                cur.execute(
                    "Update Activity \
                            SET  title = ?, Intro = ?,time = ?, area = ?, location = ?, category = ?, peopleLimited = ?, expireDate = ?, fee = ? \
                            WHERE activityID =?",
                    (
                        title,
                        Intro,
            ...
            con.commit()
        # 跳轉到 activity_detail 頁面
        return redirect("/activity/activities/activity_detail/" + id)
    except Exception as e:
        # print(e)
...
def updateComment(id):
    ...
    return render_template(
        "update_discussion.html",
        discussion=discussion,
        # time=time,
        nickname=get_nickname(discussion[0][1]),
        id=id,
    )
